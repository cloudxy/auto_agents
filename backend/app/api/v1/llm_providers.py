...
前端契约（已对齐）：
- GET /llm/providers 直出数组（无信封无分页），api_key 恒为掩码字段 api_key_masked
- PUT 时 api_key 留空表示不修改
- test 返回 {ok, latency_ms, model, error}，不落库
响应格式与 ai.py / spiders.py 一致：直接返回 domain-specific Pydantic 模型。
...
async def list_providers(
    ...
    """LLM 供应商全量列表（直出数组；api_key 恒为掩码 api_key_masked）"""
    return await service.list_providers()


@router.get("/providers/active", response_model=LlmProviderResponse)
async def get_active_provider(
    service: LlmProviderService = Depends(_service),
    _user: CurrentUser = Depends(require_login),
) -> LlmProviderResponse:
    """当前激活的 LLM 供应商（无激活行 404，此时运行时配置走 yml/env 兜底）"""
    return await service.get_active_provider()
...
@router.put("/providers/{provider_id}/activate", response_model=LlmProviderResponse)
