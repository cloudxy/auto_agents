...
> **本文档是 `docs/plan/` 下唯一的方案文档**。由 v1 三份方案（技能管理中心 / LLM 多平台模型管理 / SaaS 平台演进）+ 总览 README 于 2026-08-31 深度架构整理后合并重写而成；v1 仅存在于工作区且已随合并删除（`docs/*` 此前默认不入库，本次经 .gitignore 白名单调整首次入库）。
